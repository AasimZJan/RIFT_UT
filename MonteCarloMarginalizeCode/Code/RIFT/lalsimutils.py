    ...
    if lalsim.SimInspiralImplementedFDApproximants(P.approx)==1:
        ...
        for l in np.arange(2,Lmax+1,1):
            for m in np.arange(0,l+1,1):
                if m !=0:
                    modes_used.append((l,m))
        print(f'Based on your Lmax of {Lmax}, the possible m!=0 modes are = {list(modes_used)}')
        k = modes_to_k(modes_used)
        k_coprecessing_frame = []
        for count,value in enumerate(k):
            if value in k_coprecessing_frame_check:
...
#                'df'                 : P.deltaF,
        ...
        if not(hyp_wav):
            ...
            hpepoch = -P.deltaT*np.argmax(np.abs(hptmp)**2+np.abs(hctmp)**2)
            print(f'Calculated epoch = {hlmepoch}, SANITY check: hoft epoch would be = {hpepoch}')
...
#            hlmtmp2[mode]=np.array(hlmtmp[str(count)])
        ...
        print(f'From the model, the modes obtained are {modes_used}.\nEpoch set as={hlmepoch}.\nOnly postive m modes present={check_if_only_positive_m}')
...
#                    sys.exit()
                    ...
                    print(f'\tResizing has changed epoch to {hlm[mode].epoch}')
        ...
        if not(hyp_wav):
            ...
            TDlen_here = hlm[mode].data.length
            # Base taper, based on 1% of waveform length
