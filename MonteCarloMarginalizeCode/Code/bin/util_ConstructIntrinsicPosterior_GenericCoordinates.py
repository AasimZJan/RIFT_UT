...
def fit_nearest_neighbor(x, y, y_errors=None, fname_export=None):
    ...
    def fn_return(x_in, y=y, tree=tree, scaler=scaler):
        ...
        _, indices = tree.query(x_in_scaled)
...
def fit_knn(x, y,  y_errors=None, fname_export=None):
    from sklearn.preprocessing import MinMaxScaler
    ...
    knn.fit(x_scaled, y)
    def fn_return(x_in, y=y, knn=knn):
        # copying this from fit_rf
        f_out = -100000*np.ones(len(x_in))
        ...
        f_out[indx_ok] = knn.predict(x_in_scaled)
        return f_out
    print( " Demonstrating knn")   # debugging
    residuals = fn_return(x) - y
    print( "    std ", np.std(residuals), np.max(y), np.max(fn_return(x)))
    return fn_return
...
if not(opts.fit_load_quadratic is None):
...
elif opts.fit_method == 'xg':
    print( " FIT METHOD ", opts.fit_method, " IS xg ")
    # NO data truncation for NN needed?  To be *consistent*, have the code function the same way as the others
    X=X[indx_ok]
    Y=Y[indx_ok] - lnL_shift
    Y_err = Y_err[indx_ok]
...
elif opts.fit_method == 'knn':
    print( " FIT METHOD ", opts.fit_method, " IS knn ")
    # NO data truncation for NN needed?  To be *consistent*, have the code function the same way as the others
    X=X[indx_ok]
    Y=Y[indx_ok] - lnL_shift
    Y_err = Y_err[indx_ok]
    ...
    if opts.cap_points< len(Y) and opts.cap_points> 100:
        ...
        Y_err=Y_err[indx]
        dat_out_low_level_coord_names = dat_out_low_level_coord_names[indx]
    my_fit = fit_knn(X,Y,y_errors=Y_err)
else:
